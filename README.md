...
- В репозитории нет мусорных/битых html-дубликатов: используется только `templates/overlay.html`.
...
```bash
APP_HTTP_BIND=0.0.0.0 APP_HTTP_BIND=127.0.0.1 APP_HTTP_PORT=13337 docker compose up -d --build web redis media-storage webrtc-node coturn
```
...
```bash
NGINX_HTTP_PORT=8080 NGINX_SERVER_NAME=overlay.example.com docker compose --profile edge up -d --build
```

> Важно: если порт `80` уже занят (как у вас в логе), задайте другой, например `NGINX_HTTP_PORT=8080`.


### Доступ "только по домену" (важно для AzuraCast и multi-site VPS)

В режиме `edge` встроенный Nginx теперь использует два virtual host:
- `default_server` с `return 444` для всех неизвестных `Host` (включая заход по IP),
- отдельный `server_name` из переменной `NGINX_SERVER_NAME` для вашего домена.

Это убирает "случайную" отдачу панели по голому IP в браузере, если контейнер nginx опубликован наружу.

> Важно: это HTTP-уровень фильтрации по `Host`. Для строгого ограничения доступа к origin используйте firewall/allowlist или внешний reverse proxy.
...
## Режим с AzuraCast на том же VPS

Если AzuraCast уже занимает `80/443`, не запускайте `edge` на этих же портах. Используйте:

1. `reverse` режим для stream-overlay (`APP_HTTP_BIND=127.0.0.1`, порт 13337).
2. Один внешний nginx (или Nginx Proxy Manager), который маршрутизирует домены:
   - `radio.example.com` -> AzuraCast
   - `overlay.example.com` -> `127.0.0.1:13337`

Так вы избегаете конфликтов портов и получаете предсказуемую доменную маршрутизацию.
...
## GitHub About / описание репозитория

Важно: описание в правом блоке GitHub (**About**) не меняется автоматически от README.
Его нужно обновлять отдельно через API/интерфейс GitHub.

В репозитории есть готовые файлы/скрипты:

- `.github/repository-metadata.json` — источник описания и topics.
- `scripts/publish_github_meta.sh` — публикация metadata в GitHub About.
...
# обновить описание/темы для текущего репо
./scripts/publish_github_meta.sh

# или явно указать owner/repo
./scripts/publish_github_meta.sh Anch0vu/stream-overlay
```

---

## Проверка, что нужные файлы реально в репозитории

```bash
./scripts/verify_repo_ready.sh
```

Скрипт проверяет:
- наличие ключевых production-файлов,
- что они отслеживаются git,
- что `.sh`-скрипты не игнорируются.
...
- `APP_HTTP_BIND` — интерфейс публикации web (по умолчанию `127.0.0.1` для reverse proxy).
- `NGINX_HTTP_PORT` — внешний порт встроенного nginx (режим `edge`).
- `NGINX_SERVER_NAME` — домен для встроенного nginx в режиме `edge`.
- `NGINX_UPSTREAM` — upstream для встроенного nginx (по умолчанию `http://web:13337`).
