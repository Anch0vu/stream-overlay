...
async function PUT(url, body){ const r = await fetch(url,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); if(!r.ok) throw new Error('PUT '+url); return r.json(); }

function renderScene(){
  const surf = $("#surface"); surf.innerHTML = "";
  for(const it of state.scene.items){
    const el = document.createElement(it.kind === 'text' ? 'div' : (it.kind==='audio'?'audio': it.kind==='video'?'video':'img'));
...
function select(id){
  ...
  $("#f_x").value = it.x||0; $("#f_y").value = it.y||0;
  $("#f_w").value = it.w||0; $("#f_h").value = it.h||0;
...
function removeItem(id){
  ...
  saveScene(); renderScene();
}

function bringToFront(){
  if(!state.selected) return;
  const maxZ = state.scene.items.reduce((m,i)=>Math.max(m,i.z||1),1);
  const it = state.getItem(state.selected); it.z = maxZ+1;
  saveScene(); renderScene(); select(it.id);
}

function updateFromForm(){
  if(!state.selected) return;
  const it = state.getItem(state.selected); if(!it) return;
  it.kind = $("#f_kind").value;
  it.content = $("#f_content").value.trim();
  it.x = +$("#f_x").value||0; it.y = +$("#f_y").value||0;
  it.w = +$("#f_w").value||0; it.h = +$("#f_h").value||0;
  it.font = +$("#f_font").value||40; it.color = $("#f_color").value||"#ffffff";
...
function addItem(kind, content=''){
  const it = { id: uid(kind), kind, content, x:100, y:100, z:1, w: (kind==='text'?0:300), h:0, font:40, color:'#ffffff', bg:'transparent' };
...
$("#fileInput").addEventListener('change', async (e)=>{
  ...
  e.target.value = ""; refreshUploads();
});

loadScene(); refreshUploads();
